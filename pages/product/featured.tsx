    ...
    <>
      ...
      <main className="layout mx-auto max-w-6xl px-4 py-10">
        <div className="section-title mb-6 flex flex-wrap items-start justify-between gap-4">
          ...
          <Link className="button button-ghost rounded-full px-4 py-2 text-sm" href="/">
            Back to home
          </Link>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="empty-state rounded-2xl border border-slate-200 bg-white p-10 text-center shadow-sm">
            ...
            {user?.role === 'ADMIN' && (
              <Link className="button button-primary rounded-full px-4 py-2 text-sm font-medium" href="/admin">
                Open admin
              </Link>
            )}
          </div>
          ...
          <>
            ...
            {totalPages > 1 && (
              <div className="pagination mt-6 flex items-center justify-center gap-3">
                <button
                  className="button button-ghost rounded-full px-4 py-2 text-sm"
        ...
        {error && (
          <div className="empty-state mt-6 rounded-2xl border border-rose-200 bg-rose-50 p-8 text-center" role="status">
            <h2>Unable to load products</h2>
            <p>{error}</p>
            <Link className="button button-primary rounded-full px-4 py-2 text-sm font-medium" href="/">
              Back to home
            </Link>
          </div>
        )}
